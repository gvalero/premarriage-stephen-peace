...
const storageKey = "stephen-peace-communication-homework";
...
function formatAnswers() {
  ...
  const lines = [
    "Stephen & Peace - Communication Homework",
    `Name: ${answers.name || ""}`,
  ...
  for (const [key, value] of Object.entries(answers)) {
    if (key === "submittedAt" || key === "name") continue;
...
function downloadAnswers() {
  ...
  link.href = url;
  link.download = `${name}-communication-homework.txt`;
  link.click();
